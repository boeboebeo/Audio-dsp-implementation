"""self. -> 로 각각의 밴드의 매개변수(self.f0, self.Q ..를 사용함)
...
#class Band 보다 위에있어야 아래의 클래스에서 사용할 수 있음
...
class Band:
    def __init__(self, f0, Q, gain, filter_type):
        ...
        self.Q = Q
        self.gain = gain
        # 이 Band 의 f0, Q, gain 이란 뜻
        # 나중에 현재 계산하고 있는 그 Band 의 설정값을 가지고 옴 
        # band1 = Band(f0=_, Q=_, gain=_, filter_type="__") 이렇게 불러올 예정 
        # => band1.process(input_signal) 이런식으로 사용할 예정
...
#여기서부터 분석 코드 - 시각화
def plot_response(input_signal, output3, fs):
    ...
    #output_plot
    ax = axes[1]
    spectrum = np.fft.rfft(output3)
    magnitude = np.abs(spectrum)
    mag_db = 20 * np.log10(magnitude + 1e-12)

    fft_freq = np.fft.rfftfreq(len(output3), (1/fs))
